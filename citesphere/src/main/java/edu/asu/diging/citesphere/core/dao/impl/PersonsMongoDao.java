...
public class PersonsMongoDao implements IPersonsMongoDao {
    ...
    @Override
    public List<Persons> findPersonsByCitationGroupAndNameLikeAndUriNotIn(ICitationGroup citationGroup, String firstName,
            String lastName, List<String> uris, Pageable page) {
        UnwindOperation unwind = Aggregation.unwind("authors");
        MatchOperation match = Aggregation.match(Criteria.where("group").is(String.valueOf(citationGroup.getGroupId())).and("authors.firstName").is(firstName).and("authors.lastName").is(lastName).and("authors.uri").nin(uris).and("authors.localAuthorityId").ne(""));
        GroupOperation group = Aggregation.group("authors.uri")
                .push(new BasicDBObject("name", "$authors.name").append("uri", "$authors.uri")
                        .append("firstName", "$authors.firstName").append("lastName", "$authors.lastName")
                        .append("localAuthorityId", "$authors.localAuthorityId"))
    ...
    @Override
    public long countByPersonsByCitationGroupAndNameLikeAndUriNotIn(ICitationGroup citationGroup, String firstName, String lastName, List<String> uriList) {
        UnwindOperation unwind = Aggregation.unwind("authors");
        MatchOperation match = Aggregation.match(Criteria.where("group").is(String.valueOf(citationGroup.getGroupId())).and("authors.firstName").is(firstName).and("authors.lastName").is(lastName).and("authors.uri").nin(uriList).and("authors.localAuthorityId").ne(""));
        GroupOperation group = Aggregation.group("authors.uri")
                .push(new BasicDBObject("name", "$authors.name").append("uri", "$authors.uri")
                        .append("firstName", "$authors.firstName").append("lastName", "$authors.lastName")
                        .append("localAuthorityId", "$authors.localAuthorityId"))
    ...
    public long countByPersonsByCitationGroupAndNameLike(ICitationGroup citationGroup, String firstName, String lastName) {
        UnwindOperation unwind = Aggregation.unwind("authors");
        MatchOperation match = Aggregation.match(Criteria.where("group").is(String.valueOf(citationGroup.getGroupId())).and("authors.firstName").is(firstName).and("authors.lastName").is(lastName).and("authors.localAuthorityId").ne(""));
        GroupOperation group = Aggregation.group("authors.uri")
                .push(new BasicDBObject("name", "$authors.name").append("uri", "$authors.uri")
                        .append("firstName", "$authors.firstName").append("lastName", "$authors.lastName")
                        .append("localAuthorityId", "$authors.localAuthorityId"))
