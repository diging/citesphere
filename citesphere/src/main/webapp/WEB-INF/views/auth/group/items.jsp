    ...
    <c:forEach items="${breadCrumbs}" var="crumb">
        <c:choose>
            <c:when test="${crumb.type == 'GROUP'}">
                <c:url value="/auth/group/${crumb.objectId}/items"
                    var="crumbUrl"
                />
            </c:when>
            ...
            <c:otherwise>
                <c:url value="/auth/group/${zoteroGroupId}/items"
                    var="crumbUrl"
                />
            </c:otherwise>
        </c:choose>
    ...
    <div class="panel panel-default">
        ...
        >
            <div class="panel-body">
                ${group.description}
                <div class="col-md-6">
                    <b>Local Version:</b> ${group.version}<br> <b>Created
                        on:</b> <span class="date">${group.created}</span><br>
                    <b>Last Modified on:</b> <span class="date">${group.lastModified}</span>
                </div>
                <div class="col-md-6">
                    <b>Owner:</b> ${group.owner}<br> <br> <b>Last
                        synced:</b> <span class="date">${group.updatedOn}</span>
                </div>
            </div>
        </div>
    </div>
...
<div class="col-md-12">
    ...
    <div class="pull-right" style="margin-top: 20px;">
        <a
            href="<c:url value="/auth/group/${zoteroGroupId}/items/create" />"
            class="btn btn-primary"
        ><i class="fas fa-plus-circle"></i> Create Citation</a>
    </div>
</div>
<div class="col-md-2" id="accordion">
    <p>
        <a class="btn btn-primary dropdown-toggle" id="collapseToggle"
            data-toggle="collapse" data-parent="#accordion"
            href="#collapseCollection" aria-expanded="true"
        ...
        <ul class="list-group">
            <c:forEach items="${citationCollections}" var="collection">
                <li class="list-group-item"><span class="badge">${collection.numberOfItems}</span>
                    <a
                    href="<c:url value="/auth/group/${zoteroGroupId}/collection/${collection.key}/items" />"
        ...
        >
            <c:forEach items="${availableColumns}" var="column">
                <li><a href="#" data-column-name="${column}"
                    data-is-shown="${fn:contains(columns, column) }"
                > <spring:eval
            ...
            <tr>
                <td><span class="text-warning"> <spring:eval
                        ...
                        /> <c:if test="${empty iconClass}">
                            ...
                            />
                        </c:if> <i class="${iconClass}" title="${iconLabel}"></i>
                        ${iconLabel}
                </span></td>
                <td class="bib-entry" data-key="${entry.key}"><c:forEach
                        items="${entry.authors}" var="author"
                        varStatus="status"
                    >
                        <strong>${author.lastName}<c:if
                                test="${not empty author.firstName}"
                            >, ${author.firstName}</c:if></strong>
